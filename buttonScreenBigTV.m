...
buttonDiameter = 200;%350;%450;
buttonSpacing = 12;%15;%30;
...
while 1
    ...
    DrawFormattedText(win, 'Click on your selection (or press a number key) and then press Space.', 'center', screenBounds(4)*lowerPlacement, fontColor, wrapat);
    
    for i = 1:numButtons
        drawButton2(win, buttonDepressed(i), buttonCenters(:,i), buttonDiameter, labels{i});
        Screen('TextSize',win, 40);%100
        drawTextCenteredOnPoint(win,num2str(i-1),captionCenters(:,i));
    end
